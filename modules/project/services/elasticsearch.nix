...
let
  ...
  es7 =
    builtins.compareVersions cfg.package.version "7" >= 0
    && builtins.compareVersions cfg.package.version "8" < 0;
...
in
{
  ...
  options.services.elasticsearch = {
    ...
    dataDir = mkOption {
      type = types.path;
      default = "${config.xdg.stateHome}/elasticsearch";
      description = ''
  ...
  config = mkIf cfg.enable {
    systemd.services.elasticsearch = {
      ...
      environment = {
        ...
        ES_JAVA_HOME = pkgs.jdk17_headless;
      };
      serviceConfig = {
      ...
      preStart = ''
        ...
        install -T -m 600 ${elasticsearchYml} ${configDir}/elasticsearch.yml
        # Make sure the logging configuration for old elasticsearch versions is removed:
        rm -f "${configDir}/logging.yml"
        install -T -m 600 ${loggingConfigFile} ${configDir}/${loggingConfigFilename}
        mkdir -p ${configDir}/scripts
        install -T -m 600 ${cfg.package}/config/jvm.options ${configDir}/jvm.options
