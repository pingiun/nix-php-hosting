        ...
        name = mkOption {
          ...
          apply =
            ...
            assert (
              ...
              || abort "Project name '${x}' is longer than 31 characters which is not allowed!"
            );
            x;
  ...
  config = mkMerge [
    {
      assertions = [
        {
          assertion = all (project: !(project.mariadbVersion != null && project.mysqlVersion != null)) (
            attrValues cfg.projects
          );
          message = "Both `mysqlVersion` and `mariadbVersion` can't be set at the same time.";
        }
        {
          assertion = all (project: project.mysqlVersion != null || project.mariadbVersion != null) (
            attrValues cfg.projects
          );
        ...
        {
          assertion = all (
            project: !(project.opensearchVersion != null && project.elasticsearchVersion != null)
          ) (attrValues cfg.projects);
    ...
    (mkIf (cfg.projects != { }) {
      ...
      systemd.services = mapAttrs' (
        ...
        nameValuePair ("setup-project-${utils.escapeSystemdPath projectcfg.name}") {
          ...
          unitConfig = {
            RequiresMountsFor = config.users.users.${projectcfg.name}.home;
          };
